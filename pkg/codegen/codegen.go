...
func (c *CodeGenerator) CodegenSwitchStatement(node *parser.SwitchStatement) {
	...
	variable := c.getNewTemporary()
	c.Variables[variable] = parser.Integer
	c.output.WriteString(fmt.Sprintf("IASN %s %s\n", variable, exp.Code))

	endSwitchLabel := c.getNewLabel()
	c.breakStack = append(c.breakStack, endSwitchLabel)

	// Generate an if statement for each case
	for _, switchCase := range node.Cases {
		c.CodegenIfStatement(&parser.IfStatement{
			Condition: &parser.CompareBooleanExpression{
				LHS:      &parser.VariableExpression{Variable: variable},
				Operator: parser.EqualTo,
				RHS:      &parser.IntLiteral{Value: switchCase.Value},
			},
